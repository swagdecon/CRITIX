...
public class PersonImdbJobInfo {
    ...
    public Integer personAwardNominations(String imdbId) throws IOException, InterruptedException {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create("https://imdb8.p.rapidapi.com/actors/get-awards-summary?nconst=" + imdbId))
                .header("X-RapidAPI-Key", "349136b942msh6c632fc990fca08p1625c1jsn9838599cbe73")
                .header("X-RapidAPI-Host", "imdb8.p.rapidapi.com")
                .method("GET", HttpRequest.BodyPublishers.noBody())
                .build();
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        JsonNode rootNode = objectMapper.readTree(response.body());
        JsonNode awardsNode = rootNode.get("awardsSummary");
        Integer otherNominationsCount = awardsNode.get("otherNominationsCount").asInt();

        return otherNominationsCount;
    }
}